...
A handbook is an aggregation of Document, as there are multiple documents in each handbook, and Document is again an aggregation of versions, which keeps track of the multiple versions each document has and the period in which a version is valid within the handbook.
...
However as seen in \cref{mark-read} it is not entirely correct as there is one use case case which the administrator does not have access to.
\begin{figure}[H]
	\centering
...
The biggest similarities between the handbook-approval and supplier-approval subproblems are the need for archiving documents and the need for approving certain things with different intervals.
