...
%Taniya: husk at ændre firma til company i figuren class diagram
\todo[inline]{Draw this in tikz or something}
%Anna, jeg skal nok sætte det her op i Tikz (Kan måske nå det søndag men det afhænger af hvordan resten af ugen går)
\begin{figure}[H]
	\centering
...
\subsection{Supplier}
%Anna: Skal vi bruge samme specielle skrift her når vi omtaler classer ligesom op ved calsses and events kapitlet? (Evt bare for at tydliggøre hvornår vi snakke om klassen og om instanser af den?)

The clearer subproblem is that of managing the suppliers.
% Anna: "clearer"??? Hvad menes der med det ord i denne sammenhæng?
...
%Anna: Er det rigtig brug af aggregated er det ikke omvendt
...
The writer can do everything the reader can do, and then some.
The same goes for the administrator who can do everything the writer can do and then some.
% Taniya: forstår ikke helt "and the some", "and then some". Maybe it is just me 
%Anna: Det betyder som sådan bare at writer kan alt reader kan plus lidt ekstra osv.
...
\subsection{Approval}
The biggest similarities between the handbook and supplier-approval problem are the need for archiving documents and the need for approving certain things with different intervals.
%Anna: er det virkeligt rigtigt at kalde dem problem vil det ikke være bedre at omtale dem som domæner eller subsystemer?
...
The current version of the class diagram in \cref{fig:ClassDiagram} is a generalization used to have their shared functionality included in the abstract class Approval and the more specific functionalities in the subclasses HandbookApproval and SupplierApproval.
