...
\end{defn}
%jeg troede vi var enige om at have afskaffet denne form for citering? - astrid
...
\begin{tabular}{l l}
	...
	%Anna: Har ændret Log til Changelog
...
\end{table}
%sidste fire kandidater var skrevet med fed. jeg saa ingen grund dertil og lavede det om - astrid
...
%æhhh vi har ikke mere end et enkelt firma???
%Værd at noterer men stadig relevant ift at have den ultimative der styre tingene? (Anna)
...
%der stod item før nu staar der descriptor - astrid
...
%tilføjet noget saa vi tager hensyn til lus kommentar - astrid
%Anna: Referencen (access-archive) virker ikkke, hvad refereres der til
...
%Anna: Hvor kommer Responsibility klassen fra?
...
\begin{multicols}{4}
	...
	\end{itemize}
\end{multicols}
%Anna: Kunne det give mening at sætte dem i alfabetisk rækkefølge?
...
%det her er en fucking smuk forklaring - astrid
...
%Anna: Vil gerne udkommenterer sætningen nedenfor (da det står i den ovenfor)
...
%omformulering ovenover: begrundelse: jeg forstod det ikke -astrid
%Anna: Er ret sikker på den beskrivelse du har lavet er den der svarerer til approval requested
%Anna - hvad stod der før?, skal eventet hedde noget andet
...
%omformulering ovenover: begrundelse: jeg forstod det ikke -astrid
%Anna: Hvad var før, jeg er stærkt uenig i den nu
...
%omformulering ovenover: begrundelse: jeg forstod det ikke -astrid
%Anna: Samme igen hvad stod før er ikke sikker på jeg er enig i den nu
		...
		% Taniya: kommer til at tænke på (om det skal med) hvad hedder det event så hvis det kun er 1 bruger som har godkendt, men stadig venter på andre også godkender det?
		\textbf{Approval denied} & Approval is rejected by an approver\\
%to ovenover ændret fordi de var forkerte - astrid
...
%Anna er ikke sikker på jeg forstår formuleringen
...
%astrid: hvorfor ikke beholde denne her event i rapporten og  bare diskvalificere den fremfor at kvalificere den
%Anna Hvad mener du og hvorfor er den fjernet den er da vigtig?
...
%Anna: Igen vil mene at med den nye formuleringstemmer denher ikke længere over ens, vil gerne have formuleringen i tabellen tilbage
...
%Anna: Crucial er et meget stærkt ord da det at registrere det ikke er crucial men det at de har læst nyeste version er crucial
...
%føler lidt at argumentationen halter her - astrid
...
%Anna: Hvorfor vital; meget stærkt ord om en ting som var noget vi kom på og Ipsen så værdi i
...
\begin{multicols}{2}
	...
	%Anna: Igen hvorfor har vi fjernet den, da den er ret vigtig
	...
	\end{itemize}
\end{multicols}
\todo[inline]{under smukkeseringen s tjek lige at "overskrift" er p samme side som events}

\section{Event table}\label{sec:EventTable}
%\todo[inline]{Tjek op på definitionen skal der markeders for hvem der er påvirket før, under og efter eller kun efter?, hvis ikke finde så ekplicit skriv at den er lavet med udgangspunkt i efter eventet er tricked}
...
%Anna: Er det zero times der virkeligt menes eller up to one or multiple times der menes
...
\begin{table}[H]
	\begin{center}
		\begin{tabular}{|l|c|c|c|c|c|c|c|c|c|}
			...
			%Anna. forstår stadig ikke den er væk
			\hline
...
%Anna: Hvis tid, eller bare spørgsmål, brugeer vi det aktivt?