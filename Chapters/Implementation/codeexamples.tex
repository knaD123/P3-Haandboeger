...
As is seen on line 3 and 4 when a new version of a document is added the version's ValidFromDate is set to the current date and the \texttt{Approved} bool is set to \texttt{true}.
...
In the code examples below, \cref{lst:doccontroller} , it is shown how a \texttt{HttpGet} and \texttt{HttpPost} methods are implemented in the controller component.
...
This is done through the foreach loop from line 9 - 12.
...
In the code snippet below, \cref{lst:doccontrolleradd}, it is shown what happens when the user adds a new document or version to the system through the \texttt{Add} method.
...
In the following code snippet,\cref{lst:docview} , it is shown how the \texttt{view} component utilizes data given from the \texttt{controller} component.
\\
...
In the following code snippet, \cref{lst:docrepadd}, it is shown how a document is added to the database.
\\
...
The reason that this is important, and that one can not just use the \texttt{Document} object supplied as an argument, is due to the use of lazy-loading proxies in the application.
