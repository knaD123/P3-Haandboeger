...
This means the test subject has to use several minutes on a task.
	\item Cosmetic: When the test subject spends few minutes to completing a task and the issue is still not solved. % Anja: Jeg synes, at sætningen er forkert? Hvad skal der staa? Henrik: ændrede "need to" til "not"
\end{itemize}
...
Upload new version page and its main idea was that Ipsen would be able to mark new versions as a minor change or major change to notify readers. % Henrik: Jeg er lidt usikker paa starten af denne sætning
